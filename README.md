...
```bash
git clone https://github.com/<your-account>/keybuddy.git ~/Developer/keybuddy
cd ~/Developer/keybuddy
./setup.sh
```
...
```bash
echo 'export PATH="$HOME/Developer/keybuddy/bin:$PATH"' >> ~/.zshrc
```

(Optional) Source the bash function wrapper for shorter commands:

```bash
echo 'source ~/Developer/keybuddy/lib/keybuddy.sh' >> ~/.zshrc
```
...
| Roo Code | `<project>/.roo/rules/keybuddy.md` | Project |
| Cline | `<project>/.clinerules/keybuddy.md` | Project |
...
mkdir -p ~/.claude/skills/keybuddy
cp agent-skill/claude-code/SKILL.md ~/.claude/skills/keybuddy/

# Cursor (per project)
mkdir -p .cursor/rules
cp agent-skill/cursor/keybuddy.mdc .cursor/rules/

# Universal AGENTS.md (per project — covers most agents)
cp agent-skill/keybuddy.md AGENTS.md
```

Cursor User Rules and Windsurf Custom Instructions are GUI-only — copy-paste the contents of [`agent-skill/keybuddy.md`](agent-skill/keybuddy.md) into the app's settings panel.

## Versioning and updates

```bash
secrets --version    # → keybuddy 0.1.0
cat VERSION          # → 0.1.0
```
...
rm -rf ~/.config/keybuddy
rm -rf ~/Developer/keybuddy
```
