...
BANNER
    ...
    echo "==========================================="
    echo ""
    echo "When you 'source lib/secrets-helper.sh' in your shell rc, secrets-helper can show"
    echo "a one-line load confirmation:"
    echo ""
    echo "  secrets-helper 0.1.0 loaded — try \`tiers\` to see your configured tiers"
    echo ""
    ...
    {
        echo "# secrets-helper preferences"
        echo "# Generated by setup.sh on $(date '+%Y-%m-%d %H:%M:%S')"
        echo "# Edit to change shell-load behavior."
        echo ""
        if [[ "$banner_choice" == "2" ]]; then
            echo "SECRETS_HELPER_QUIET=1   # silent load"
        else
            echo "# SECRETS_HELPER_QUIET=1   # uncomment to suppress load banner"
        fi
    } > "$prefs_file"
