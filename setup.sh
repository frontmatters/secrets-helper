...
readonly CONFIG_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/keybuddy"
...
detect_agents() {
    [[ -d "$HOME/.claude" ]]   && echo "Claude Code|$HOME/.claude/skills/keybuddy/SKILL.md|global"
    [[ -d "$HOME/.kiro" ]]     && echo "Kiro|$HOME/.kiro/steering/keybuddy.md|global"
    command -v cursor   >/dev/null 2>&1 && echo "Cursor|.cursor/rules/keybuddy.mdc|project"
...
install_global_skill() {
    ...
    case "$agent" in
        ...
        "Kiro"|"Codex CLI") source_file="$SKILL_DIR/keybuddy.md" ;;
...
print_project_install_instructions() {
    ...
    while IFS='|' read -r agent target type; do
        ...
        local source_file
        case "$agent" in
            Cursor) source_file="$SKILL_DIR/cursor/keybuddy.mdc" ;;
            *)      source_file="$SKILL_DIR/keybuddy.md" ;;
        esac
...
BANNER
    ...
    echo ""
    echo "==========================================="
    echo "  Shell preferences"
    echo "==========================================="
    echo ""
    echo "When you 'source lib/keybuddy.sh' in your shell rc, keybuddy can show"
    echo "a one-line load confirmation:"
    echo ""
    echo "  keybuddy 0.1.0 loaded — try \`tiers\` to see your configured tiers"
    echo ""
    echo "Show this banner?"
    echo "  1) yes — show one-line banner on every shell start"
    echo "  2) no  — silent load (no output)"
    local banner_choice
    banner_choice=$(prompt_choice "Choice" "1")

    local prefs_file="$CONFIG_DIR/preferences.sh"
    {
        echo "# keybuddy preferences"
        echo "# Generated by setup.sh on $(date '+%Y-%m-%d %H:%M:%S')"
        echo "# Edit to change shell-load behavior."
        echo ""
        if [[ "$banner_choice" == "2" ]]; then
            echo "KEYBUDDY_QUIET=1   # silent load"
        else
            echo "# KEYBUDDY_QUIET=1   # uncomment to suppress load banner"
        fi
    } > "$prefs_file"
    echo "  Preferences written to $prefs_file"
    ...
    echo "==========================================="
    echo ""
    echo "AI coding agents work better when they know about keybuddy."
    ...
    echo "  2. (Optional) source:  source $SCRIPT_DIR/lib/keybuddy.sh"
