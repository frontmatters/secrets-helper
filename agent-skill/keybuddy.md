...
This document teaches you how to use `keybuddy`, a tiered macOS Keychain wrapper. When this tool is installed on the user's machine, you should use it as the **single source of truth** for API tokens, credentials, and signing keys — not `.env` files, not chat, not memory.
